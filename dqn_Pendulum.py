...
class FXTrade(gym.core.Env):
    def __init__(self):
        ...
        for i in range(len(x)):
            self._sin_list.append(x[i])
            sin_original.append(x[i])
...
plt.ylabel("pos")
plt.pause(3)
plt.savefig('plot_epoch_{0:03d}_pendulum_dqn.png'.format(9600), dpi=60)
plt.close()
...
plt.savefig('plot_epoch_{0:03d}_pendulum_obvs&original.png'.format(9600), dpi=60)
plt.close()
